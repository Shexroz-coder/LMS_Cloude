...
export async function getFinanceTotals(): Promise<FinanceTotals> {
  const [activeAgg, allAgg, debtorCount] = await Promise.all([
    ...
    // Barcha o'quvchilar (ketganlar bilan)
    prisma.studentBalance.aggregate({
      _sum: { debt: true },
    }),
 ...
 */
export async function getTotalIncome(from?: Date, to?: Date): Promise<number> {
 ...
 * Xarajat (expenses).
 */
export async function getTotalExpenses(from?: Date, to?: Date): Promise<number> {
